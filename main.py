...
def generate_pdf_file(inv_no, items):
    ...
    # 2. ข้อมูลการขนส่ง (ปรับปรุงลำดับ 2.1 ตามที่ระบุ)
    c.drawString(1.2*cm, h-10.7*cm, "2. ข้อมูลการขนส่ง")
    
    # 2.1 ผู้ดำเนินการขนส่ง (6 รายการ)
    ...
    # 2.2 พนักงานขับรถ (ขยับตำแหน่ง X เล็กน้อยเพื่อความสวยงาม)
    ...
    # 3. รายละเอียดน้ำมันเชื้อเพลิง (ขยับลงตามข้อมูลข้อ 2.1 ที่เพิ่มขึ้น)
    c.drawString(1.2*cm, h-14.5*cm, "3. รายละเอียดน้ำมันเชื้อเพลิง")
    ...
    t.wrapOn(c, 1*cm, h-18.0*cm) # ปรับตำแหน่งวาดตารางลง
    t.drawOn(c, 1*cm, h-18.0*cm)

    # 4. การยืนยันและรับสินค้า
    c.setFont(FONT_NAME, 11)
    c.drawString(1.2*cm, h-19.0*cm, "4. การยืนยันและรับสินค้า")
    c.setFont(FONT_NAME, 10)
    c.drawString(1.5*cm, h-19.6*cm, "ข้าพเจ้าได้รับสินค้าตามรายการข้างต้นในสภาพเรียบร้อย ถูกต้องตามจำนวนและหมายเลขซีลที่ระบุไว้")

    # --- 5. ลายเซ็น ---
    sig_y = (h - 24.5*cm) # ปรับตำแหน่งลายเซ็นลงเล็กน้อย
